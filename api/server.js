...
app.all("/api/server", async (req, res) => {
  ...
  try {
    ...
    if (route === "stock" && req.method === "POST") {
      ...
      if (!product.length) {
        product = await sql`
          ...
          VALUES (${name},${price},${expiry_date}) RETURNING id
        `;
      }
      ...
      if (!stock.length) {
        ...
        await sql`
          UPDATE stock SET quantity=quantity+${quantity} WHERE product_id=${pid}
        `;
      }
    ...
    if (route === "sales" && req.method === "POST") {
      ...
      if (borrow_amount > 0) {
        ...
        if (!b.length) {
          ...
          await sql`
            UPDATE borrowers SET outstanding_amount=outstanding_amount+${borrow_amount}
            WHERE id=${b[0].id}
          `;
        }
      }
    ...
    if (route === "borrower-payments") {
      ...
      `;
      await sql`
        UPDATE borrowers SET outstanding_amount=outstanding_amount-${amount}
